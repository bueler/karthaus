...
While we should probably not trust any computer program for any purpose, the above verification is a start.  Another test is of ``robustness'': does the model break when you ask it to do hard things, even if you don't know exactly what it should do.  Here we demonstrate that \texttt{siaflat.m} can handle an ice sheet with extraordinarily high driving stresses.  Recall that glaciological ``driving stress'' is $\tau_d = \rho g H \grad h$, which appears in the slab-on-a-slope example and the SIA model as the value of the shear stress $(\tau_{13},\tau_{23})$ at the base of the ice.

Specifically, the program \texttt{roughice.m} gives the numerical code \texttt{siaflat.m} the worst-possible ice sheet, which is both thick and has large surface slopes, shown in the left side of Figure \ref{fig:roughinitial}.  After 50 model years of evolution the ice cap has the shape rather close to a Halfar solution state, as also shown in Figure \ref{fig:roughinitial}.  The first time step was determined adaptively as 0.0002 years, and this increases in the adaptive time-stepping scheme to close to the user-chosen maximum step of 0.2 years.

\twofig{roughinitial}{roughfinal}{FIXME}
