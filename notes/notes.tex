...
Code \texttt{diffusion.m} solves the general diffusion equation $T_t = F + \Div \left(D(x,y) \grad T\right)$.  The user supplies the diffusivity $D(x,y)$ on the staggered grid, and the initial temperature $T(0,x,y)$ and heat source $F(x,y)$ on the regular (i.e.~non-staggered) grid.
