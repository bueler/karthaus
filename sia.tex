...
\hspace{-10mm}\includegraphics[width=1.3\textwidth]{heatconduction}
\end{column}
\end{columns}
\end{frame}
...
\item how do we make sure an \emph{implemented} numerical scheme is correct?
  ...
  \item<2->[$\circ$] \emph{technique} 2: compare your model with others, and hope that the outliers are the ones with errors \only<2>{$=$ \alert{intercomparison}}
  \item<3>[$\circ$] \emph{technique} 3: compare your model to an exact solution, and actually measure the numerical error \only<3>{$=$ \alert{verification}}
  \end{itemize}
\end{itemize}
\end{frame}
