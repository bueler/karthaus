...
How does the ice area $A$ in the Figure change?\footnote{In three-dimensions it would be ``how does the ice volume change?''}  We add the boundary contributions,
\begin{equation}
...
\subsection*{The hydrostatic and Blatter approximations}  We return now briefly to equations \eqref{incompressiblexz}--\eqref{forceflowz}, and reconsider how to simplify them.  One method, present in all shallow models, is the ``hydrostatic'' approximation which drops a single term in the $z$-component of the stress balance \eqref{stokespz}.  It assumes that $\tau_{13,x}$, the horizontal variation in the vertical shear stress, is small compared to the other terms.  Thus
\begin{equation}
p_z = - \tau_{11,z} - \rho g. \label{hydrostaticpz}
\end{equation}
Because the (Cauchy) stress tensor $\sigma_{ij}$ is related to the deviatoric tensor by $\sigma_{ij} = \tau_{ij} - p \delta_{ij}$, and thus $p + \tau_{11} = p - \tau_{33} = - \sigma_{33}$, equation \eqref{hydrostaticpz} says that the vertical normal stress $\sigma_{33}$ is linear in depth.  Taking it to have surface value zero we get
\begin{equation}
p + \tau_{11} = \rho g (h-z). \label{hydrostaticitself}
\end{equation}
FIXME: also rewrite in viscosity form

FIXME: get hydrostatic equation
\begin{equation}
\rho g h_x = \left(4 \nu u_x\right)_x + \left(\nu (u_z+w_x)\right)_z \qquad\text{\emph{hydrostatic stress balance}} \label{stresshydrostatic}
\end{equation}
FIXME: drop $w_x$ to get Blatter equations
\begin{equation}
\rho g h_x = \left(4 \nu u_x\right)_x + \left(\nu u_z)\right)_z \qquad\text{\emph{Blatter stress balance}} \label{stressblatter}
\end{equation}
FIXME: in both models get $w$ from incompressibility \eqref{incompressiblexz}; all details are an exercise
