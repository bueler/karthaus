...
FIXME explicit scheme in two space dimensions

recall heat equation in 2D: $u_t = D(u_{xx} + u_{yy})$

in two spatial variables we write $u_{jk}^n \approx u(t_n,x_j,y_k)$

the 2D explicit scheme is
\begin{equation}
	FIXME \frac{u_{jk}^{n+1} - u_{jk}^n}{\Delta t} = D\,\left(\frac{u_{j+1,k}^n - 2 u_{jk}^n + u_{j-1,k}^n}{\Delta x^2} + \frac{u_{j,k+1}^n - 2 u_{jk}^n + u_{j,k-1}^n}{\Delta y^2}\right)  \label{heat2dexplicit}
\end{equation}
