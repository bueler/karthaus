...
The stability criteria is most easily guaranteed by making each step short enough to satisfy it.  This is an \textsl{adaptive} explicit implementation, and it has guaranteed stability.  It is simple to implement, but if the diffusivity $D$ is very large then it will take short time steps.  To show how easy it is to implement, see \texttt{heatadapt.m} below.  It is the same as \texttt{heat.m} except that the time step is chosen from the stability criterion.  It will never show the instability as seen in Figure \ref{fig:stability}; try it and see!

\minput{heatadapt}

There is an alternative stability fix, however, ``implicitness.''  For example, the finite difference scheme
   $$\frac{T_j^{n+1} - T_j^n}{\Delta t} = D\,\frac{T_{j+1}^{n+1} - 2\, T_j^{n+1} + T_{j-1}^{n+1}}{\Delta x^2}$$
is an implicit scheme for Equation \eqref{heat1D}.  When written as a formula for $T_j^{n+1}$ there are only positive coefficients, so no stability criterion needs to be satisfied for the scheme to be an average.  Implicit schemes for the heat equation are stable for \emph{any} positive time step $\Delta t>0$ (``unconditionally stable'').  A well-know example implicit scheme is \emph{Crank-Nicolson}.  It is unconditionally stable for the heat equation and it has smaller error too; the error is $O((\Delta t)^2,(\Delta x)^2)$.

But implicit schemes are a bit harder to implement.  The values at time step $t_{n+1}$ are generally treated as a vector of unknowns in a large system of equations.  This system of equations must be formed and solved at each time step.  If the PDE is nonlinear---the SIA is a kind of nonlinear heat equation---the system of equations may be hard to solve.  For the purposes in these notes we stick with the adaptive explicit idea, and it works fine.  More generally, there is a tradeoff between the implementability of adaptive explicit schemes and the stability of implicit schemes.\footnote{Computer scientist Donald Knuth has advice for ice sheet modellers, among other people: ``We should forget about small efficiencies \dots: premature optimization is the root of all evil.''}
