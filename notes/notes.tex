...
So now we jump forward from 1830 to 1981, the year in which P.~Halfar published the similarity solution of the SIA in the case of flat bed and no surface mass balance.
...
\end{equation}
where $r=(x^2+y^2)^{1/2}$ is distance from the origin.  It is shown in Figure \ref{fig:siascaling}.  The scalings in \eqref{halfarscalings} imply that, quite differently from heat, the diffusion of ice really slows down as the shape flattens out!  That is, the powers $t^{-1/9}$, $t^{-1/18}$ imply very little change for times $t$ significantly larger than one.  This is what we see in the Figure.

\onefigsize{siascaling}{Halfar's solution $H(t,r)$ of the SIA equation \eqref{sia} on a flat bed (i.e.~$b=0$ so $H=h$) and with zero mass balance (i.e.~$M=0$).  The solution is shown on $H$ versus $r$ axes for times $t=1,10,100,1000,10000$ years.}{5.5in}
...
Thus if we choose central height $H_0$ and ice cap radius $R_0$ then the characteristic time $t_0$ is determined, and, at that time $t=t_0$, the Halfar solution has height $H_0$ and radius $R_0$.
%EXERCISE: verify these simple claims

Formula \eqref{halfar} is certainly simple enough to use for verifying time-dependent SIA models, and so we use it that way.  Among the distributed codes is \texttt{verifysia.m}, which takes as input the number of grid points in each ($x,y$) direction.  It uses the Halfar solution at 200 a as the initial condition, does a numerical run to time 20000 a, with \texttt{siaflat.m} above, and then compares to the Halfar formula for that time:
\small
\begin{verbatim}
>> verifysia(20)
...
\end{verbatim}
\normalsize
We see that the thickness error, the absolute value of the difference between the numerical and exact solutions at $t=20000$ a, is shrinking with increasing grid resolution.  This is as expected for a correctly-implemented code.  What is less obvious, perhaps, is that almost any numerical implemention error will break this property and we will find that these errors do not shrink as we expect them to.

You might ask, is the Halfar solution good for modelling real ice masses directly?  John Nye and others (2000)
...
compared the long-time consequences of different flow laws for the South Mars Polar Cap.  In particular, they evaluated $\text{CO}_2$ ice and $\text{H}_2\text{O}$ ice softness parameters by comparing the long-time behavior of the corresponding Halfar solutions.  Their conclusions:
  ...
  \end{quote}
This theoretical result has increasing confirmation in the observation of, and sampling of, the polar geology of Mars.


\subsubsection*{Exact solutions for ice flow models}
