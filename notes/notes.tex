...
\subsubsection*{Numerical solution of diffusion equations}  Recall we are trying to numerically model ice flows, not heat conduction.  But we have an analogy which says that the SIA is like the heat equation, as both are diffusions.  In fact the heat equation \eqref{heat} has the more common name of ``diffusion equation,'' as the unknown may not be a temperature.  In this section, because we wish to solve the SIA, we construct a numerical scheme for an even more general diffusion which has a ``shift'' inside the gradient, namely
\begin{equation}
  T_t = F + \Div \left(D \grad (T + b)\right). \label{gendiffusion}
\end{equation}

The previous code \texttt{heatadapt.m} assumes $D$ is constant and does not have $F,b$ at all, but now suppose the source term $F(x,y)$, the diffusivity $D(x,y)$, and the ``shift'' $b(x,y)$ all vary in space.    An adaptive explicit method for the diffusion equation is conditionally stable, with the same time step restriction as for the constant $D$ case, as long as we evaluate diffusivity $D(x,y)$ at \emph{staggered} grid points:
\begin{align*}
...
\end{align*}
where $X=T+b$.  Figure \ref{fig:diffstencil} shows the stencil for this diffusion equation approximation; the staggered grid points are shown with triangles.

\twofig{diffstencil}{mahaffystencil}{Left:  Spatial stencil for staggered grid evaluation of diffusivity in the diffusion equation \eqref{heat}.  Right: Stencil showing how the staggered-grid diffusivity is evaluated in the Mahaffy choice.  Triangles are the staggered grid locations where $D$ is evaluated (left) or computed (right).}

Code \texttt{diffusion.m} below solves the general diffusion equation \eqref{gendiffusion}.  The user supplies the diffusivity $D(x,y)$ on the staggered grid, and the initial temperature $T(0,x,y)$, the source term $F(x,y)$, and the ``shift'' $b(x,y)$ on the regular (i.e.~non-staggered) grid.
...
But in practice, as long as we are stepping forward in time and not computing steady states directly, % N&R: cite Jouvet & Bueler
it suffices to numerically compute new thicknesses and then set them to zero if they come out negative.

As noted, for numerical stability we compute the diffusivity $D$ on the staggered grid.  Various schemes have been proposed---see Notes and References---
% \cite{Mahaffy,vanderVeen,HindmarshPayne}
but all involve averaging $H$ and differencing $h$ in a ``balanced'' way, for better accuracy, onto the staggered grid.  In these notes we use the Mahaffy (1976) choice, with the diffusivity-compuation stencil shown in Figure \ref{fig:diffstencil}.

Our SIA implementation in the flat bed, zero surface mass balance case is \texttt{siaflat.m} below.  The general SIA solver, which solves Equation \eqref{sia} including general bed elevation and spatially-variable surface mass balance, is \texttt{siageneral.m} (not shown).
...
The last one of these is called ``verification.''  % for N&R: cite Roache
We need to return once again to the PDEs themselves, to get exact solutions to help test our codes.
