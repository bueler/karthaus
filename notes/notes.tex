...
\subsubsection*{A first model for the Antarctic ice sheet}  Finally we might really use the model, in this case for the Antarctic ice sheet.  We carefully make some small modifications of \texttt{siaflat.m} to get \texttt{siageneral.m}.  We use observed accumulation as surface mass balance, allow a non-flat bedrock (so $H\ne h$)---we simply use BEDMAP \cite{BamberLayberryGogenini}, we modify the surface gradient method to be FIXME: floatkill

 differentiate the surface correctly where floating, and

NO: calve at current calving front location

here are results from this \emph{toy} Antarctic flow model

a 2000 model year run on a $\Delta x=50$ km grid; runtime a few seconds

\twofig{antinitial}{antfinal}{FIXME}
