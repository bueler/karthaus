...
\twofigsizes{initialheat}{finalheat}{Left: initial condition $T(0,x,y)$.   Right: solution $T(t,x,y)$ at $t=0.02$.  (In fact these pictures are computed numerically.)}{2.8in}{2.8in}
...
For the first Matlab/Octave implementation we go directly to the heat equation in two spatial dimensions, Equation \eqref{heat}, but with $D$ constant and $F=0$.  In two spatial variables we write $T_{jk}^n \approx T(t_n,x_j,y_k)$.  The 2D explicit scheme is
\begin{equation}
...
\end{Verbatim}
The second one gives the look of instability.  Both figures compute $T$ on the same spatial grid ($40\times 40$), for the same final time $t_f=0.05$, but with slightly different time steps.

\twofig{stability}{instability}{Everything in these two runs is the same, except left has $\Delta t=0.0005$, $D\Delta t\Delta x^2= 0.2$ and right has $\Delta t=0.001$, $D\Delta t/\Delta x^2= 0.4$.}
