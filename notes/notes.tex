...
\end{equation}
The {\color{red} \textbf{red} term} is the vertically-integrated ``longitudinal'' or ``membrane'' stress, the {\color{blue} \textbf{blue} term} is the basal resistance, and the negative of the {\color{green} \textbf{green} term} is the driving stress.  Thus we can say that the SSA is stress balance where longitudinal stresses are determined by the slipperyness of the bed (i.e.~the coefficient $C$) and the geometry of the ice sheet (i.e.~the thickness $H$ and the surface slope $h_x$).

How should we think about this equation?  How should we solve it numerically?  We address these questions, though not exhaustively, below.
...
\subsubsection*{Steady ice shelf exact solution}  exact velocity and thickness for steady ice shelf:
\begin{itemize}
