...
\rho \left(\mathbf{u}_t + \mathbf{u}\cdot\nabla \mathbf{u}\right) &= -\nabla p + \nabla \cdot \tau_{ij} + \rho \mathbf{g} &&\text{\emph{force balance (``$m a = F$'')}} \\
...
\item and from ``$u_z = 2 A \tau^2 \tau_{13}$'' get velocity formula
\vspace{-0.05in}
\begin{align*}
...
\small
\begin{itemize}
\item ``area'' here becomes ``volume'' in 3D
\end{itemize}
\end{frame}
