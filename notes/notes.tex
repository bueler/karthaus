...
\subsubsection*{Numerical solution of diffusion equations}  Recall we are trying to numerically model ice flows, not heat.  In fact, we had an analogy
    $$\text{SIA} \leftrightarrow \text{heat eqn}$$
where the heat equation had form $T_t = F + \Div \left(D(x,y) \grad T\right)$.  A more common name for this general heat equation is ``diffusion equation''.  Finally, we had identified formula for the SIA's diffusivity $D$, in terms of ice thickness and surface slope.

So here we consider numerically solving a diffusion equation in which the diffusivity $D$ varies in space: $D=D(x,y)$, because the previous code \texttt{heatadapt.m} assumes $D$ is constant.  An adaptive explicit method for the diffusion equation \eqref{heat} is conditionally stable, with the same time step restriction as for constant $D$, if we evaluate diffusivity $D(x,y)$ at \alert{staggered} grid points:
\begin{align*}
...
\end{align*}
Figure \ref{fig:diffstencil} shows the stencil for this diffusion equation approximation.
...
Code \texttt{diffusion.m} solves the general diffusion equation $T_t = F + \Div \left(D(x,y) \grad T\right)$.  The user supplies the diffusivity $D(x,y)$ on the staggered grid, plus the initial temperatures $T(0,x,y)$ and the heat source $F(x,y)$ on the regular (i.e.~non-staggered) grid.

\minput{diffusion}

FIXME:  on ``degenerate'' diffusivity
