...
\begin{equation}
  H_t = M + \left(\frac{2}{5} (\rho g)^5 A H^5 |h_x|^2 h_x\right)_x. \label{sia1D}
\end{equation}
...
Additional arguments are needed to actually show that the SIA is ``general purpose'' and not just special to a simple slab.  See Notes and References for these general arguments that derive the SIA model from the Stokes equations, under the assumptions that slopes are small and the depth to width ratio is also small.

We will numerically solve the SIA equation in the next section.  First we seek to understand it a bit better, and state it in two horizontal dimensions.
...
Combining Equations \eqref{siavelocity} and \eqref{masscont} and defining a positive constant $\Gamma = 2 A (\rho g)^n / (n+2)$, we get an equation how thickness changes:
\begin{equation}
H_t = M + \Div \left(\Gamma H^{n+2} |\grad h|^{n-1} \grad h \right) \label{sia}
\end{equation}
This is the SIA equation we will solve numerically.  To review the notation, $H$ is ice thickness, $h$ is ice surface elevation, $b$ is bed elevation ($h=H+b$), and $M$ combines surface and basal mass (im)balance.

Fulfilling an earlier promise, if we just solve \eqref{sia} numerically then we get a usable model for the Barnes ice cap, see for example \cite{Mahaffy}.  It is a particularly simple ice sheet on a very flat bed with minimal evidence for sliding.


\subsubsection*{Analogy with the heat equation}  An obligatory step to applying the SIA model is to understand that Equation \eqref{sia} is analogous to the better known heat equation.  All numerical methods for solving \eqref{sia} can be understood as modifications of well-known heat equation methods.
...
where $T$ is object temperature and $K$ relates to material and geometric properties of an object (e.g.~a cup of coffee).  That is, the temperature of an object tends to decay exponentially to the temperature of its surroundings (``ambient temperature'').  One way to derive the heat equation, which is a partial differential equation, is to argue that the segments of a rod also satisfy Newton's law of cooling.  They experience ambient temperatures which are the average temperature of the segment's neighbors:
...
The more complete 2D version of the heat equation describes the temperature $T(t,x,y)$ at position $x,y$ in an object and at time $t$.  Fourier rewrote Newton's law as a rule for heat flux, namely $\mathbf{q} = - k \grad T$.  If we allow an additional heat source $f$, conservation of energy gives us the standard heat equation:
...
\end{tabular}
\end{center}
Notice that the number of derivatives (one time and two space derivatives) and the signs are the same in these equations.  Clearly mass balance $M$ is analogous to (scaled) heat sources $F$.  The analogy allows us to identify the diffusivity in the SIA:
	$$D = {\color{red}\Gamma H^{n+2} |\grad h|^{n-1}}.$$
The non-sliding shallow ice flow \alert{diffuses} the ice sheet, and when $\Gamma$ or $H$ or $|\grad h|$ are large, the diffusion acts more quickly. 
...
As noted, numerical schemes for the heat equation are a good starting place for solving the SIA equation \eqref{sia} numerically.  Here we describe only so-called finite difference schemes, with discussion of alternatives left to the Notes and References.
...
The actual idea in finite differences is to combine expressions like these to give approximations of derivatives, from the values of the unknown quantity at locations on a grid.  Furthermore we want partial derivative expressions, for example with a general function $u=u(t,x)$:
\begin{align*}
...
\end{align*}
and so on.  Sometimes we want a derivative in-between grid points:
	$$u_x(t,x+(\Delta x/2)) = \frac{u(t,x+\Delta x) - u(t,x)}{\Delta x} + O(\Delta x^2)$$
Note that ``$+O(\Delta^2)$'' is better than ``$+O(\Delta)$'' if $\Delta$ is a small number.
...
\subsection{Numerically solving the SIA}
