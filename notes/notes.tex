...
%EXERCISE: The other two coefficients are positive because we step forward in time (so $\mu= D \Delta t / (\Delta x)^2>0$).\footnote{Running the heat equation backward in time, or using a negative value for $D$, are bad ideas.  Try it and see!}
...
%EXERCISE: check the above claims
...
% EXERCISE: do this 1D heat equation verification
...
%EXERCISE: verify these simple claims
...
%EXERCISE: try this
%EXERCISE: some output from verifysia.m supressed; e.g. the max thickness error does not shrink monotonically; why?  why the spatial distribution of max error?

You might ask, is the Halfar solution also good for modelling real ice masses?  John Nye and others (2000)
%N&R: \cite{NyeIcarus2000}
...
\end{equation}
The {\color{red} \textbf{red} term} is the vertically-integrated ``longitudinal'' or ``membrane'' stress, the {\color{blue} \textbf{blue} term} is the basal resistance, and the negative of the {\color{green} \textbf{green} term} is the driving stress.  Thus we can say that the SSA is stress balance where longitudinal stresses are determined by the slipperyness of the bed (i.e.~the coefficient $C$) and the geometry of the ice sheet (i.e.~the thickness $H$ and the surface slope $h_x$).  Note that the velocity $u(x)$ is independent of the vertical coordinate $z$.

How should we think about this equation?  How should we solve it numerically?  We address these questions---not exhaustively, but concretely---below.

\subsubsection*{Flotation and grounding lines}  The equation simplifies if the ice is floating, because we assume zero resistance is applied by the ocean (and the atmosphere, for that matter).  Also the ice surface elevation is determined by flotation.  The inequality ``$\rho H < - \rho_w b$'' is sometimes called the \emph{flotation criterion}.  At the grounding line $x=x_g$ the inequality switches.  On the grounded side we know $\rho H > - \rho_w b$ and that the driving stress is $\rho g H h_x = \rho g H (H_x + b_x)$.  On the floating side we know $\rho H < - \rho_w b$ and that $h = (1-\rho/\rho_w) H$ so the driving stress is $\rho g H h_x = \rho(1-\rho/\rho_w) g H H_x$.  Thus the SSA becomes
\begin{equation}
   \left(2 A^{-1/n} H |u_x|^{1/n - 1} u_x\right)_x = \rho g (1-\rho/\rho_w) H H_x \label{ssafloat}
\end{equation}
for floating ice.  A useful observation about equation \eqref{ssafloat} is that both left and right sides are derivatives, because $(H^2)_x = 2 H H_x$; we'll use this below.

\subsubsection*{Steady ice shelf exact solution}  In fact, because of the relative simplicity of the SSA equation \eqref{ssafloat} for 1D floating ice, the exact velocity and thickness for steady ice shelf can be computed \cite{vanderVeen83}.  Note that for such a steady ice shelf with $H_t=0$, the mass continuity equation \eqref{masscont} reduces to $M=(uH)_x$.  To compute the exact solution we need the ice thickness $H_g$ and ice velocity $u_g$ at the grounding line and we set the surface mass balance $M$ to be a positive constant $M_0$.  The derivation of this solution, which is left to the exercises, uses the above-mentioned ``useful observation.''
%EXERCISE: do it

Supposing $H_g=500$ m, $u_g = 50 \,\text{m}\,\text{a}^{-1}$, and $M_0=30 \,\text{cm}\,\text{a}^{-1}$ we get the result in Figure \ref{fig:steadyshelfprofile}.  We will use this exact solution to understand the SSA better and to verify a numerical SSA code.

\twofig{steadyshelfprofile}{steadyshelfvelocity}{Results from \texttt{exactshelf.m}.  Left:  Upper and lower surface elevation (m a.s.l.) of van der Veen exact ice shelf solution.  Right: Velocity (m/a) for same solution.}

\subsubsection*{Numerical solution of the SSA}  Suppose the ice thickness is a fixed function $H(x)$.  To find the velocity numerically we solve the nonlinear PDE \eqref{ssa} for the unknown $u(x)$.  An iteration is needed, because of the nonlinearity.  We describe  numerical method for a shelf \emph{or} stream, but only give a code for an ice shelf, and leave the ice stream modifications as an exercise.

First note that coefficient $\bar \nu = A^{-1/n} |u_x|^{1/n-1}$ in \eqref{ssa} is the ``effective viscosity'', so that \eqref{ssa} can be written
   $$\left(2 \,\bar \nu\, H u_x\right)_x - C |u|^{m-1} u = \rho g H h_x.$$
The simplest iteration idea is to use the old effective viscosity to get new velocity solution, and repeat until things stop changing.  This is a ``Picard'' iteration; Newton iteration is a superior alternative but we stick to Picard in these notes.

Specifically, denote the previous iterate as $u^{(k-1)}$ and the current iterate---the unknown---as $u^{(k)}$.  Define $W^{(k-1)} = 2 \bar \nu H = 2 A^{-1/n} |u^{(k-1)}_x|^{1/n-1} H$.  Now solve repeatedly:
     $$\left(W^{(k-1)} u^{(k)}_x\right)_x - C |u^{(k-1)}|^{m-1} u^{(k)} = \rho g H h_x.$$
At each iteration this is a linear problem, sometimes called the ``inner'' iteration.  In the abstract it is a linear boundary value problem
\begin{equation}
  \left(W(x)\, u_x\right)_x - \alpha(x)\, u = \beta(x)  \label{innerlinear}
\end{equation}
on an interval $0 < x < L$, with boundary conditions $u(0) = V$ and $u_x(L) = \gamma$.  (FIXME: explain calving front b.c.)  It is an \emph{elliptic} PDE boundary value problem.  The functions $W(x)$, $\alpha(x)$, $\beta(x)$ are known in this SSA context because both $W(x)$ and $\alpha(x)$ come from the previous iteration, while $\beta(x)$ is also known; it is the driving stress.

Where do you get an initial guess $u^{(0)}$?  We can give a helpful answer in the 1D cases.  For floating ice, a possible initial guess for velocity comes from assuming a uniform strain rate: $u^{(0)}(x) = \gamma (x-x_g) + u_g$, where $\gamma$ is the value of $u_x$ found from calving front condition.  For grounded ice, a possible initial guess for velocity is to assume ice is held by basal resistance only: $u^{(0)}(x) = \left(-C^{-1} \rho g H h_x\right)^{1/m}$.

\subsubsection*{Numerics of the ``inner'' linear problem}  Suppose we discretize the $x$-axis with equal spacing $\Delta x$ and index $j=1,2,\dots,J+1$, so that $x_1 = x_g$ and $x_{J+1} = x_c$ are endpoints.  The coefficient $W(x)$ is needed on the staggered grid.  The approximation of \eqref{innerlinear} is
\begin{equation}
  \frac{W_{j+1/2} (u_{j+1} - u_j) - W_{j-1/2} (u_{j} - u_{j-1})}{\Delta x^2} - \alpha_j u_j = \beta_j  \label{discreteinnerlinear}
\end{equation}
For the left end boundary condition we have $u_1 = V$ given; this will be easy to include in the linear system (below).

For the right end boundary condition we have ``$u_x(L)=\gamma$'' given.  This requires more thought \cite{MortonMayers}.  First introduce a notional point $x_{J+2}$.  Now require $(u_{J+2} - u_J)/(2 \Delta x) = \gamma$.  Using equation \eqref{discreteinnerlinear} in $j=J+1$ case, we can now eliminate the $u_{J+2}$ variable ``by-hand'', before coding the numerics.
...
There are special cases in the last equation:
$$A_{J+1,J} = 2 W_{J+1/2}$$
$$A_{J+1,J+1} = -(2 W_{J+1/2}+\alpha_{J+1}\Delta x^2)$$
$$b_{J+1} = -2 \gamma \Delta x W_{J+3/2} + \beta_{J+1} \Delta x^2$$
This is a \emph{tridiagonal} system.

\minput{flowline}

\subsubsection*{Testing the parts}  We test the ``inner'' linear code before proceeding to solve the nonlinear SSA problem.  We can test the ``abstracted'' code \texttt{flowline.m} by ``manufacturing'' solutions.  FIXME: See \texttt{testflowline.m}; not shown.  This shows (FIXME: not shown) that our implemented numerical scheme converges at the optimal rate $O(\Delta x^2)$.

The following code computes the thickness and velocity for a steady ice shelf numerically.  That is, it implements the Picard iteration to solve the nonlinear equation \eqref{ssa}.

\minput{ssaflowline}

A convergence analysis of \texttt{ssaflowline.m} using the van der Veen exact solution described above yields Figure \ref{fig:shelfconv}.  (See \texttt{testshelf.m}.)

\onefig{shelfconv}{FIXME}
...
\onefig{marineice}{FIXME: from Grosfeld \& Thyssen 1994} \nocite{GrosfeldThyssen1994}
...
\twofig{rossquiver}{rossscatter}{Left: FIXME.  Right: FIXME.}
...
Which are the best numerical models for moving grounding lines, even when the minimal SSA stress balance is used, is still an open question \cite{SchoofMarine1,MISMIP2012}.  The physics requires at least that the quantities $H,u,u_x$ are all continuous at $x=x_g$.
