...
\exer{1}{Derive the Green's function of the heat equation.}

\exer{2}{Assume $u$ has continuous second derivatives.  Show
  $$u_x(t,x) = \frac{u(t,x+\Delta x) - u(t,x-\Delta x)}{2\Delta x} + O(\Delta x^2).$$
}
...
\exer{5}{Is P.~Halfar male or female, and what does the first initial ``P.'' stand for?}

\exer{6}{The code \texttt{verifysia.m} also shows the numerical error in the ice thickness in a map-plane view.  Near the grounded margin of an ice sheet this error is much larger than elsewhere.  Why?}

\exer{7}{Get the surface kinematical equation from the mass continuity and basal kinematical equations.}
