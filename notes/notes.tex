...
As already noted, numerical schemes for the heat equation are a good starting place for solving the SIA equation \eqref{sia} numerically.  Here we describe only finite difference schemes, with numerical alternatives left to the Notes and References.

Finite difference schemes replace derivatives in the differential equation by arithmetic.  The basic fact is \emph{Taylor's theorem}.  It says that for differentiable $f(x)$ and any real number $\Delta$,
...
The idea for constructing finite difference schemes is to combine expressions like these to give approximations of derivatives, using values of the unknown quantity only at locations on a grid.  Furthermore we want partial derivative expressions, but this merely means we apply the Taylor's expansions one variable at a time.  For example with a general function $u=u(t,x)$ we can write several approximations.
\begin{align*}
...
\end{align*}
and so on.  Note that ``$+O(\Delta^2)$'' is better than ``$+O(\Delta)$'' if $\Delta$ is a small number.  Sometimes we also want a derivative in-between grid points:
	$$u_x(t,x+(\Delta x/2)) = \frac{u(t,x+\Delta x) - u(t,x)}{\Delta x} + O(\Delta x^2)$$

Now we can give an explicit scheme for the 1D heat equation, the PDE \eqref{heat1D}.  In terms of the values of the exact solution $T(t,x)$ we would say
...
How accurate is \eqref{heat1Dfd}?  The conclusion we can draw from the above construction of is that the scheme \eqref{heat1Dfd} and the PDE \eqref{heat1D} is $O(\Delta t,\Delta x^2)$ (``consistency'').  With care about the smoothness of boundary conditions, and using the mathematical theory of the heat equation itself, the well-established theory of finite difference schemes shows that the difference between $T_j^n$ and $T(t_n,x_j)$ is also $O(\Delta t,\Delta x^2)$ (``convergence'').  But this latter fact is less automatic.  On the one hand the PDE must be well understood.  On the other hand, the scheme \eqref{heat1Dfd} must be stable, which we address below.  In fact, the main theorem of the field of numerical PDEs is ``consistency plus stability implies convergence''.  But we leave such theory for other reading.  % FIXME: cite \cite{MortonMayers}

Even for the first Matlab/Octave implementation, we will go directly to the heat equation in two spatial dimensions, namely \eqref{heat}.  Note that in two spatial variables we write $T_{jk}^n \approx T(t_n,x_j,y_k)$.  The 2D explicit scheme is
\begin{equation}
...
\end{equation}
The stencil for the spatial derivative approximation is shown in Figure \ref{fig:expstencil}, right.
...
Scheme \eqref{heat2dexplicit} has the implementation \texttt{heat.m} in Matlab/Octave.  It numerical solves PDE \eqref{heat} on the square $-1 < x < 1$, $-1 < y < 1$ using a gaussian initial condition: $T(0,x,y) = e^{-30 r^2}$.  It also uses ``colon notation'' to remove loops over spatial variables.  For example,
\begin{Verbatim}
>>  heat(1.0,30,30,0.001,20)}
\end{Verbatim}
approximates $T$ on a $30\times 30$ spatial grid, with $D=1$ and $N=20$ steps of $\Delta t = 0.001$.  The result is shown in Figure \ref{fig:initialheat}, right.  This is the look of success.

\minput{heat}



FIXME from here:  the look of instability
